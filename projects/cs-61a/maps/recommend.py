...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            ratings += [user_rating(user, restaurant_name(restaurant))]
        else:
            ratings += [predictor(restaurant)]
    return dict(zip([restaurant_name(restaurant) for restaurant in restaurants], ratings))
    # END Question 9
