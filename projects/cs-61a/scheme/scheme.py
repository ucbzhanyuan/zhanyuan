...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        values = Pair(scheme_eval(bindings.first.second.first, env), values) # why is not bingdings.first.second here?
        bindings = bindings.second
    check_formals(formals)
    child_frame = env.make_child_frame(formals, values)
    return child_frame
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    special = expressions.first
    check_form(special, 1)
    if not scheme_symbolp(special.first):
        raise SchemeError()
    env.define(special.first, MacroProcedure(special.second, expressions.second, env))
