...
(quote (define jim-pitman (/ 1 0)))
; expect (define jim-pitman (/ 1 0))
...
(begin (begin 3 4) (define now_u_see_me (begin 3 4)))
;expect now_u_see_me
...
(add-7 3)
; expect 10
(define (sum-add-7 x y)
        (+ (add-7 x) (add-7 y)))
; expect sum-add-7
(sum-add-7 134 135)
; expect 283

(define x ((lambda (y) (* y 3)) 62))
; expect x
x
; expect 186
...
(define (begin x) (begin (define x 4) (define y 6)))
;expect begin
...
(define (cool_stuff stuff) (even? stuff))
;expect cool_stuff


cool_stuff
;exepct (lambda (stuff) (even? stuff))
...
(and False (and (and (/ 1 0))))
; expect #f

(and (define x 2) (eval (define y x)))
; expect 2

(and (quote a) (define x a))
; expect Error

(if (and 189 False) (/ 1 0) (quote y))
; expect y
...
(cond 
  ((< 1 1) (and (define x 2) (eval (define y x))))
  ((= 1 1) (or (and (define x 5) (eval (define y x))) (define y (+ x 5)) (/ 1 0)))
  (else (/ 1 0)))
; expect 5

(define (have-same-elems sorted_lst1 sorted_lst2) ; two sorted lists in increasing order
  (cond 
    ((or (null? sorted_lst1) (null? sorted_lst2)) False)
    ((= (car sorted_lst1) (car sorted_lst2)) True)
    ((> (car sorted_lst1) (car sorted_lst2)) (have-same-elems sorted_lst1 (cdr sorted_lst2)))
    ((< (car sorted_lst1) (car sorted_lst2)) (have-same-elems (cdr sorted_lst1) sorted_lst2))))
; expect have-same-elems
(have-same-elems '(1 4 7 5) '(7 8))
; expect #t
(have-same-elems '(1 3 5 7) '(2 4 6 8))
; expect #f
...
(define inner (mu (x y z) (+ (* 100 x) (* 10 y) z)))
; expect inner
(define (outter q) (inner q q q))
; expect outter
(outter 1)
; expect 111
...
(define g (mu (mew) (mew)))
; expect g
(define (mew) (/ 1 0)) ; mew in global frame
; expect mew
(let ((mew (lambda () 2)))
     (g mew))
; expect 2
